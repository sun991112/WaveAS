...
from wavegc_model import MODEL_REGISTRY, build_model
...
def save_wavegc_spectral_visualizations(
    trace_records,
    output_dir,
    ...
    np.savez(
        os.path.join(vis_dir, "wavegc_spectral_trace.npz"),
        epochs=epochs,
        eigvals=eigvals,
    ...
    plt.xlabel("Eigenvalue")
    plt.ylabel("Energy")
    plt.title("Spectral Energy Before and After WaveGC Spectral Block")
    plt.legend()
    plt.tight_layout()
    ...
    return {
        "trace_npz": os.path.join(vis_dir, "wavegc_spectral_trace.npz"),
...
def save_wavegc_ad_analysis_figures(
    trace_records,
    output_dir,
    node_influence_context=None,
    case_selection_context=None,
    ...
    trace_npz_path = os.path.join(vis_dir, "wavegc_ad_trace.npz")
    np.savez(
        trace_npz_path,
        epochs=epochs,
        eigvals=eigvals,
...
def train_one_fold(
    ...
    relation_profiles = None
    node_influence_context = None
    if use_spectral_context and model_name == "wavegc_spectral":
        spectral_eigvals, spectral_eigvecs = compute_spectral_context_from_adjacency(
            train_adj_matrix,
            topk=spectral_topk,
        )
        if visualize_wavegc_spectral and model_name == "wavegc_spectral":
    ...
    for epoch in range(1, epochs + 1):
        ...
        if batch_size is None or batch_size >= len(train_edges):
            ...
            if hasattr(model, "diversity_regularization") and wavegc_diversity_lambda > 0:
                loss = loss + wavegc_diversity_lambda * model.diversity_regularization()
            if hasattr(model, "wavelet_energy_regularization") and wavegc_wavelet_energy_lambda > 0:
                loss = loss + wavegc_wavelet_energy_lambda * model.wavelet_energy_regularization()
            if hasattr(model, "wavelet_scaling_balance_regularization") and wavegc_wavelet_balance_lambda > 0:
                loss = loss + wavegc_wavelet_balance_lambda * model.wavelet_scaling_balance_regularization(
                    target_ratio=wavegc_wavelet_target_ratio
                )
            loss.backward()
            optimizer.step()
        else:
            ...
            for batch_edges, batch_labels in iterate_edge_batches(
                ...
                if hasattr(model, "diversity_regularization") and wavegc_diversity_lambda > 0:
                    batch_loss = batch_loss + wavegc_diversity_lambda * model.diversity_regularization()
                if hasattr(model, "wavelet_energy_regularization") and wavegc_wavelet_energy_lambda > 0:
                    batch_loss = batch_loss + wavegc_wavelet_energy_lambda * model.wavelet_energy_regularization()
                if hasattr(model, "wavelet_scaling_balance_regularization") and wavegc_wavelet_balance_lambda > 0:
                    batch_loss = batch_loss + wavegc_wavelet_balance_lambda * model.wavelet_scaling_balance_regularization(
                        target_ratio=wavegc_wavelet_target_ratio
                    )
                batch_loss.backward()
                optimizer.step()
        ...
        if (
            visualize_wavegc_spectral
            and model_name == "wavegc_spectral"
    ...
    if visualize_wavegc_spectral and model_name == "wavegc_spectral" and visualization_output_dir:
        result["visualization_paths"] = save_wavegc_ad_analysis_figures(
            spectral_trace_records,
...
def run_cv_with_full_graph(
    ...
    for fold_idx, (train_pos_edges, test_pos_edges, train_neg_edges, test_neg_edges) in enumerate(folds, start=1):
        ...
        result = train_one_fold(
            ...
            visualize_wavegc_spectral=visualize_wavegc_spectral,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_layers", type=int, default=2, help="Number of stacked WaveGC spectral blocks.")
    ...
    parser.add_argument("--model_name", type=str, default="wavegc_spectral", choices=["wavegc_spectral"])
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=True,
        help="Use truncated eigenvalues/eigenvectors from each training graph to condition the WaveGC kernel generator.",
    )
    parser.add_argument(
        "--spectral_topk",
        type=int,
    ...
    parser.add_argument(
        "--wavegc_diversity_lambda",
        type=float,
        default=1e-2,
        help="Regularization weight that discourages different WaveGC wavelet branches from collapsing to the same order distribution.",
    )
    parser.add_argument(
        "--wavegc_wavelet_energy_lambda",
        type=float,
        default=1e-2,
        help="Regularization weight that encourages WaveGC spectral wavelet branches to keep non-trivial response magnitude.",
    )
    parser.add_argument(
        "--wavegc_wavelet_balance_lambda",
        type=float,
        default=1e-1,
        help="Regularization weight that prevents WaveGC spectral wavelet filters from collapsing to a tiny fraction of the scaling filter.",
    )
    parser.add_argument(
        "--wavegc_wavelet_target_ratio",
        type=float,
        default=0.25,
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=False,
        help="Record and save spectral filter visualizations during training when model_name=wavegc_spectral.",
    )
    parser.add_argument(
        "--visualize_every",
        type=int,
        default=5,
        help="Save one WaveGC spectral visualization snapshot every N epochs.",
    )
    parser.add_argument(
        "--use_eigen_encoding",
        type=str2bool,
        default=True,
        help="Use the WaveGC-style spectral eigenvalue encoder before decoding scaling/wavelet coefficients and scales.",
    )
    parser.add_argument(
        "--use_local_mpnn",
        type=str2bool,
        default=False,
        help="Enable a local GCN message-passing branch inside each WaveGC spectral block, following the original local+global fusion pattern.",
    )
    parser.add_argument(
    ...
    common_kwargs = dict(
        ...
        visualize_wavegc_spectral=args.visualize_wavegc_spectral,
